...
class RideCompleteFragment : Fragment() {
    ...
    private fun completeAndGoHome(driverName: String, fare: Double, destName: String, rating: Int) {
        val completedRide = com.coride.data.model.Ride(
            id = "ride_${System.currentTimeMillis()}",
            pickup = com.coride.data.model.Place("p1", "Pickup", "Current Location", 31.5204, 74.3587),
            destination = com.coride.data.model.Place("p2", destName, destName, 0.0, 0.0),
            driver = com.coride.data.repository.MockDataRepository.getDrivers().firstOrNull { it.name == driverName },
            status = com.coride.data.model.RideStatus.COMPLETED,
            requestedFare = fare,
            finalFare = fare,
        ...
        com.coride.data.repository.MockDataRepository.addRide(completedRide)
