package com.indrive.clone.data.repository

import com.indrive.clone.data.model.*
import com.indrive.clone.utils.EmailNotificationHelper
import kotlinx.coroutines.delay

object MockDataRepository {

    private var currentUser = com.indrive.clone.data.local.LocalPreferences.getUser() ?: User()
    ...
    fun updateUser(user: User) {
        currentUser = user
        com.indrive.clone.data.local.LocalPreferences.saveUser(user)
    }
    ...
    fun completeVerification() {
        ...
        com.indrive.clone.data.local.LocalPreferences.saveUser(currentUser)
    ...
    fun completeRegistration(otp: String): Boolean {
        if (verifyOtp(otp)) {
            com.indrive.clone.data.local.LocalPreferences.saveRegistrationData(pendingEmail, pendingPhone, pendingPass, pendingStudentId)
            val newUser = User(
    ...
    fun login(loginId: String, pass: String): Boolean {
        val regEmail = com.indrive.clone.data.local.LocalPreferences.getRegisteredEmail()
        val regPhone = com.indrive.clone.data.local.LocalPreferences.getRegisteredPhone()
        val regPass = com.indrive.clone.data.local.LocalPreferences.getRegisteredPassword()
        ...
        if ((emailMatch || phoneMatch) && regPass == pass) {
            ...
            val prefs = com.indrive.clone.data.local.LocalPreferences
    ...
    fun isLoggedIn(): Boolean = com.indrive.clone.data.local.LocalPreferences.getUser() != null

    fun logout() {
        verificationTimerRunning = false
        com.indrive.clone.data.local.LocalPreferences.clearUser()
        currentUser = User()
    }
    ...
    fun deleteAccount() {
        ...
        // Wipe all local data
        com.indrive.clone.data.local.LocalPreferences.clearAllData()
    ...
    fun sendResetOtp(identity: String): Boolean {
        val regEmail = com.indrive.clone.data.local.LocalPreferences.getRegisteredEmail()
        val regStudentId = com.indrive.clone.data.local.LocalPreferences.getRegisteredStudentId()
    ...
    fun finalizePasswordReset(newPass: String) {
        ...
        com.indrive.clone.data.local.LocalPreferences.saveRegistrationData(regEmail, regPhone, newPass)
        
        // Security: Disable biometrics after password reset
        com.indrive.clone.data.local.LocalPreferences.setBiometricEnabled(false)
    ...
    private val apiService by lazy {
        ...
        retrofit2.Retrofit.Builder()
            ...
            .build()
            .create(com.indrive.clone.data.api.WeatherApiService::class.java)
    }
                ...
                return list.filterIndexed { index, _ -> index % 8 == 0 } // Roughly 1 entry per day
                    ...
                    .map { item ->
                        ...
                        val iconRes = when (item.weather.firstOrNull()?.icon?.take(2)) {
                            "01" -> com.indrive.clone.R.drawable.ic_sunny
                            "02", "03", "04" -> com.indrive.clone.R.drawable.ic_cloudy
                            "09", "10", "11" -> com.indrive.clone.R.drawable.ic_rainy
                            else -> com.indrive.clone.R.drawable.ic_cloudy
                        }
    ...
    fun getWeatherForecast(): List<WeatherDay> {
        return listOf(
            WeatherDay("Mon", "28°C", com.indrive.clone.R.drawable.ic_sunny, "Sunny"),
            WeatherDay("Tue", "26°C", com.indrive.clone.R.drawable.ic_cloudy, "Cloudy"),
            WeatherDay("Wed", "24°C", com.indrive.clone.R.drawable.ic_rainy, "Rainy"),
            WeatherDay("Thu", "25°C", com.indrive.clone.R.drawable.ic_cloudy, "Partly Cloudy"),
            WeatherDay("Fri", "27°C", com.indrive.clone.R.drawable.ic_sunny, "Sunny"),
            WeatherDay("Sat", "29°C", com.indrive.clone.R.drawable.ic_sunny, "Hot"),
            WeatherDay("Sun", "22°C", com.indrive.clone.R.drawable.ic_rainy, "Stormy")
        )
    }
}
